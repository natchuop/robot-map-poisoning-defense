...
For the fake-obstacle experiment shell, `bash scripts/runTestFakeObstacle.sh` launches `webots/worlds/TestFakeObstacle/TestFakeObstacle.wbt` with the shared-mapping launch path, the static `/map` layer, and two RViz windows. It also starts the click-to-inject fake obstacle node automatically, so you can publish points in RViz without opening a second terminal. Both robot windows can publish fake reports, and both windows mirror the merged live map, trust-weighted confidence heatmap, and the gray floor underneath.

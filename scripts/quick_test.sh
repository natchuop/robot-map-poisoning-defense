...
WORLD_HAS_SECONDARY_ROBOT=false
if grep -q '"bridge_port":[[:space:]]*5006' "$WORLD_PATH" 2>/dev/null; then
  WORLD_HAS_SECONDARY_ROBOT=true
fi
...
if [ "$TEST_MODE" = "amcl" ]; then
  docker_compose run -d \
    ...
    -e RMPD_MAP_ID="${RMPD_MAP_ID:-${WORLD_BASENAME%.wbt}}" \
    -e RMPD_CHECKPOINT_METRICS_RUN_NAME="${RMPD_CHECKPOINT_METRICS_RUN_NAME:-quick_test}" \
    -e RMPD_AMCL_MAP_NAME="${RMPD_AMCL_MAP_NAME:-$AMCL_MAP_BASENAME}" \
    -e RMPD_BRIDGE_PORT_SECONDARY="${RMPD_BRIDGE_PORT_SECONDARY:-}" \
    -e RMPD_START_SECONDARY_BRIDGE="${RMPD_START_SECONDARY_BRIDGE:-$WORLD_HAS_SECONDARY_ROBOT}" \
    -e RMPD_SECONDARY_BRIDGE_TOPIC_PREFIX="${RMPD_SECONDARY_BRIDGE_TOPIC_PREFIX:-robot_2}" \

#!/usr/bin/env bash
set -euo pipefail

REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

export RMPD_TEST_MODE="multi_mapping"
export RMPD_WEBOTS_WORLD="$REPO_DIR/webots/worlds/routeMultipleRobots/routeMultipleRobots.wbt"
export RMPD_MULTI_ROBOT_CONFIG="$REPO_DIR/src/robot_patrol_node/config/route_multiple_robots_config.json"
export RMPD_ROUTE_CONFIG="$REPO_DIR/src/robot_patrol_node/config/route_multiple_robots_routes.json"
export RMPD_RVIZ_CONFIG_FILES="${RMPD_RVIZ_CONFIG_FILES:-multi_robot_robot_1_view.rviz,multi_robot_robot_2_view.rviz,multi_robot_robot_3_view.rviz,multi_robot_robot_4_view.rviz}"
export RMPD_RVIZ_WINDOW_COUNT="${RMPD_RVIZ_WINDOW_COUNT:-4}"
export RMPD_START_CHECKPOINT_PATROL="false"
export RMPD_START_MULTI_ROUTE_FOLLOWERS="true"
export RMPD_START_NAVIGATION_DIAGNOSTICS="false"
export RMPD_START_LIVE_MAPPING="true"
export RMPD_ROBOT_1_COMPROMISED="${RMPD_ROBOT_1_COMPROMISED:-true}"
export RMPD_ROBOT_2_COMPROMISED="${RMPD_ROBOT_2_COMPROMISED:-true}"
export RMPD_ROBOT_3_COMPROMISED="${RMPD_ROBOT_3_COMPROMISED:-false}"
export RMPD_ROBOT_4_COMPROMISED="${RMPD_ROBOT_4_COMPROMISED:-false}"
export RMPD_ROBOT_1_TRUST_WEIGHT="${RMPD_ROBOT_1_TRUST_WEIGHT:-1.0}"
export RMPD_ROBOT_2_TRUST_WEIGHT="${RMPD_ROBOT_2_TRUST_WEIGHT:-0.35}"
export RMPD_ROBOT_3_TRUST_WEIGHT="${RMPD_ROBOT_3_TRUST_WEIGHT:-1.0}"
export RMPD_ROBOT_4_TRUST_WEIGHT="${RMPD_ROBOT_4_TRUST_WEIGHT:-1.0}"
export RMPD_FAKE_REPORT_RADIUS_CELLS="${RMPD_FAKE_REPORT_RADIUS_CELLS:-2}"
export RMPD_FAKE_OBSTACLE_INJECTOR_MODE="${RMPD_FAKE_OBSTACLE_INJECTOR_MODE:-clicked_point}"
export RMPD_QUICK_TEST_FORCE_CLEAN="${RMPD_QUICK_TEST_FORCE_CLEAN:-true}"
export RMPD_LIVE_MAP_WIDTH_M="${RMPD_LIVE_MAP_WIDTH_M:-10.0}"
export RMPD_LIVE_MAP_HEIGHT_M="${RMPD_LIVE_MAP_HEIGHT_M:-10.0}"
export RMPD_LIVE_MAP_ORIGIN_X="${RMPD_LIVE_MAP_ORIGIN_X:--4.0}"
export RMPD_LIVE_MAP_ORIGIN_Y="${RMPD_LIVE_MAP_ORIGIN_Y:--4.0}"
export RMPD_BRIDGE_PORT="${RMPD_BRIDGE_PORT:-5005}"
export RMPD_BRIDGE_PORT_SECONDARY="${RMPD_BRIDGE_PORT_SECONDARY:-5006}"
export RMPD_BRIDGE_PORT_THIRD="${RMPD_BRIDGE_PORT_THIRD:-5007}"
export RMPD_BRIDGE_PORT_FOURTH="${RMPD_BRIDGE_PORT_FOURTH:-5008}"

exec bash "$REPO_DIR/scripts/quick_test.sh"

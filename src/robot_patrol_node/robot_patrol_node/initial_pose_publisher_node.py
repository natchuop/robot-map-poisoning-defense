...
class InitialPosePublisherNode(Node):
    """Publish an AMCL initial pose from odom or configured coordinates."""
    ...
    def publish_pose(self) -> None:
        ...
        if pose_values is None:
            if not self.waiting_logged:
                self.get_logger().info(
                    f'Waiting for {self.odom_topic} before publishing initial pose.'
                )
        ...
        self.sent_count += 1
        self.get_logger().info(
            f'Published initial pose #{self.sent_count}: '
            f'({x:.2f}, {y:.2f}, {yaw:.2f})'
        )

...
class CheckpointPatrolNode(Node):
    def __init__(self):
        ...
        self.webots_checkpoint_event_pub = self.create_publisher(String, '/webots_checkpoint_event', 10)
        ...
        self.get_logger().info(
            f'Checkpoint route: {" -> ".join(VISIBLE_ROUTE)}; arrival radius={self.goal_reached_radius:.2f} m'
        )
        self.get_logger().info(
            'Visible checkpoints complete only after Webots reports robot footprint contact with the colored block.'
        )
    ...
    def checkpoint_contact_callback(self, msg):
        ...
        if isinstance(distance, (int, float)):
            reason = f'touching colored block in Webots at {float(distance):.2f} m from marker center'
        else:
    ...
    def feedback_callback(self, feedback_msg, sequence):
        ...
        if self.last_progress_distance is None or progress_distance < self.last_progress_distance - 0.15:
        ...
        if should_log:
            ...
            if distance is None:
                self.get_logger().info(
                    f'Checkpoint {self.active_checkpoint_name}: Nav2 reports {nav2_remaining:.2f} m remaining'
                )
            else:
                self.get_logger().info(
                    f'Checkpoint {self.active_checkpoint_name}: {distance[0]:.2f} m from marker ({distance[1]})'
                )
    ...
    def result_callback(self, future, sequence):
        ...
        if result.status == GoalStatus.STATUS_SUCCEEDED:
            ...
            if distance is None:
                ...
                self.retry_active_checkpoint(
                    f'Nav2 succeeded but helper is still {distance[0]:.2f} m away using {distance[1]}'
                )
            return
    ...
    def complete_active_checkpoint(self, reason):
        ...
        if checkpoint_name in HELPER_TARGETS:
            target_name = HELPER_TARGETS[checkpoint_name]
            message = f'PASSED waypoint for checkpoint {target_name}: {checkpoint_name} ({reason})'
        else:
            checkpoint_number = sum(
                1 for name in self.route[: self.current_index + 1] if name not in HELPER_TARGETS
            )
            total = len(VISIBLE_ROUTE)
            message = f'REACHED checkpoint {checkpoint_number}/{total}: {checkpoint_name} ({reason})'
        print(message, flush=True)
        self.get_logger().info(message)
